...
const loadFileType = async () => {
    const mod = await Promise.resolve().then(() => import('file-type'));
    const api = mod?.default ?? mod;
    const fromBuffer = api?.fromBuffer ?? api?.fileTypeFromBuffer;
    if (!fromBuffer) {
        throw new Error('file-type missing fromBuffer');
    }
    return { fromBuffer };
};
const MIMETYPE_MAP = {
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (typeof uploadData.media === 'string') {
        uploadData.media = { url: uploadData.media };
    }
    ...
    if (!uploadData.mimetype || (mediaType === 'document' && uploadData.mimetype === MIMETYPE_MAP.document)) {
        const { fromBuffer } = await loadFileType();
        let mediaBuffer;
        if (Buffer.isBuffer(uploadData.media)) {
            mediaBuffer = uploadData.media;
        }
        else if (uploadData.media && typeof uploadData.media === 'object' && 'url' in uploadData.media) {
            const urlValue = (uploadData.media.url?.toString?.() || '').trim();
            if (urlValue.startsWith('data:')) {
                const commaIndex = urlValue.indexOf(',');
                if (commaIndex !== -1) {
                    const meta = urlValue.slice(5, commaIndex);
                    const data = urlValue.slice(commaIndex + 1);
                    mediaBuffer = meta.includes(';base64')
                        ? Buffer.from(data, 'base64')
                        : Buffer.from(decodeURIComponent(data), 'utf8');
                }
            }
            else if (urlValue.startsWith('base64:')) {
                mediaBuffer = Buffer.from(urlValue.slice('base64:'.length), 'base64');
            }
            else if (/^[A-Za-z0-9+/=\r\n]+$/.test(urlValue) && !urlValue.includes('/') && !urlValue.includes('\\') && urlValue.length > 128) {
                mediaBuffer = Buffer.from(urlValue, 'base64');
            }
        }
        if (mediaBuffer) {
            const fileType = await fromBuffer(mediaBuffer);
            if (fileType?.mime) {
                uploadData.mimetype = fileType.mime;
            }
            if (mediaType === 'document' && !uploadData.fileName && fileType?.ext) {
                uploadData.fileName = `file.${fileType.ext}`;
            }
        }
    }
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        const hasExternalAdReply = !!(message.contextInfo && message.contextInfo.externalAdReply);
        if (!hasExternalAdReply && typeof urlInfo === 'undefined') {
