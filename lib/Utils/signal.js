...
const parseAndInjectE2ESessions = async (node, repository) => {
    ...
    for (const nodesChunk of chunks) {
        await Promise.all(nodesChunk.map(async (node) => {
            ...
            const decodedJid = (0, WABinary_1.jidDecode)(jid);
            const registrationId = (0, WABinary_1.getBinaryNodeChildUInt)(node, 'registration', 4);
            if (!jid || !(decodedJid === null || decodedJid === void 0 ? void 0 : decodedJid.user) || !identity) {
                return;
            }
            if (typeof repository.validateSession === 'function') {
                const existing = await repository.validateSession(jid);
            ...
            try {
                ...
                // swallow invalid session injections to avoid blocking other sessions
            }
        }));
    }
};
