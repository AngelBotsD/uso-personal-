...
const uploadWithNodeHttp = async ({ url, filePath, headers, timeoutMs, agent }, redirectCount = 0) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const stream = (0, fs_1.createReadStream)(filePath, { highWaterMark: 1024 * 1024 });
        stream.pipe(req);
        stream.on('error', err => {
            req.destroy();
            reject(err);
...
const uploadWithFetch = async ({ url, filePath, headers, timeoutMs, agent }) => {
    const nodeStream = (0, fs_1.createReadStream)(filePath, { highWaterMark: 1024 * 1024 });
    // @ts-ignore
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options, useHostrtaCdn }, refreshMediaConn) => {
    return async (fileOrStream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        try {
            ...
            const fileStats = await fs_1.promises.stat(filePath);
            const effectiveTimeoutMs = timeoutMs !== null && timeoutMs !== void 0 ? timeoutMs : Math.min(30 * 60_000, Math.max(Defaults_1.UPLOAD_TIMEOUT, Math.ceil(fileStats.size / (1024 * 1024)) * 1000));
