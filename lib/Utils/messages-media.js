...
exports.uploadBufferToHostrtaCdn = uploadBufferToHostrtaCdn;
...
async function uploadBufferToHostrtaCdn(buffer, filename, expiresIn = '10m') {
    const { fromBuffer } = await Promise.resolve().then(() => __importStar(require('file-type')));
    const fileType = await fromBuffer(buffer);
    const mime = (fileType === null || fileType === void 0 ? void 0 : fileType.mime) || 'application/octet-stream';
    const ext = (fileType === null || fileType === void 0 ? void 0 : fileType.ext) || 'bin';
    const finalFilename = filename || `${Date.now()}-${Crypto.randomBytes(3).toString('hex')}.${ext}`;
    const form = new form_data_1.default();
    form.append('files', buffer, { filename: finalFilename, contentType: mime });
    if (expiresIn) {
        form.append('expiresIn', expiresIn);
    }
    const uploadResponse = await axios_1.default.post('https://cdn.hostrta.win/upload', form, {
        headers: {
            ...form.getHeaders(),
    ...
    const file0 = Array.isArray(data === null || data === void 0 ? void 0 : data.files) ? data.files[0] : null;
    return (file0 === null || file0 === void 0 ? void 0 : file0.publicUrl) || ((file0 === null || file0 === void 0 ? void 0 : file0.url) ? `https://cdn.hostrta.win${file0.url.startsWith('/') ? '' : '/'}${file0.url}` : null);
}
...
const parseInlineBase64 = (value) => {
    ...
    const isLikelyPath = trimmed.includes('/') || trimmed.includes('\\');
    if (looksLikeBase64 && !isLikelyPath && trimmed.length > 128) {
        try {
...
const getStream = async (item, opts) => {
    ...
    const inlineBuffer = parseInlineBase64(urlValue);
    if (inlineBuffer) {
        return { stream: (0, exports.toReadable)(inlineBuffer), type: 'buffer' };
    }
...
const uploadWithNodeHttp = async ({ url, filePath, headers, timeoutMs, agent }, redirectCount = 0) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const stream = (0, fs_1.createReadStream)(filePath, { highWaterMark: 4 * 1024 * 1024 });
        stream.pipe(req);
        stream.on('error', err => {
            req.destroy();
            reject(err);
...
const uploadWithFetch = async ({ url, filePath, headers, timeoutMs, agent }) => {
    const nodeStream = (0, fs_1.createReadStream)(filePath, { highWaterMark: 4 * 1024 * 1024 });
    // @ts-ignore
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options, useHostrtaCdn }, refreshMediaConn) => {
    return async (fileOrStream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        try {
            ...
            const fileStats = await fs_1.promises.stat(filePath);
            const effectiveTimeoutMs = timeoutMs === 0 || Defaults_1.UPLOAD_TIMEOUT === 0
                ? undefined
                : (timeoutMs !== null && timeoutMs !== void 0 ? timeoutMs : Math.min(2 * 60 * 60_000, Math.max(Defaults_1.UPLOAD_TIMEOUT, Math.ceil(fileStats.size / (1024 * 1024)) * 1000)));
            const uploadToHost = async ({ hostname, maxContentLengthBytes }) => {
            ...
            const batchSize = Math.min(2, hosts.length);
            for (let i = 0; i < hosts.length; i += batchSize) {
                const batch = hosts.slice(i, i + batchSize);
                try {
                    // race a small batch to reduce tail latency
                    urls = await Promise.any(batch.map(uploadToHost));
                    break;
                }
                catch (error) {
                    const isLastBatch = i + batchSize >= hosts.length;
                    logger.warn({ trace: error === null || error === void 0 ? void 0 : error.stack }, `Error in uploading to ${batch.map(h => h.hostname).join(', ')} ${isLastBatch ? '' : ', retrying...'}`);
                }
            }
            if (!urls) {
                throw new boom_1.Boom('Media upload failed on all hosts', { statusCode: 500 });
            }
            if (useHostrtaCdn && (urls === null || urls === void 0 ? void 0 : urls.mediaUrl)) {
                let cdnUrl;
                try {
                    const response = await fetch('https://cdn.hostrta.win/upload-url', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ url: urls.mediaUrl, expiresIn: '5m' })
                    });
                    const data = await response.json();
                    const publicUrl = data === null || data === void 0 ? void 0 : data.files?.[0]?.publicUrl;
                    if (publicUrl) {
                        cdnUrl = publicUrl;
                    }
                }
                catch (_b) { }
                if (!cdnUrl && fileStats.size <= 20 * 1024 * 1024) {
                    try {
                        const buffer = await fs_1.promises.readFile(filePath);
                        cdnUrl = await uploadBufferToHostrtaCdn(buffer, undefined, '10m');
                    }
                    catch (_c) { }
                }
                if (cdnUrl) {
