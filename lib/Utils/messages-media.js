...
exports.getStatusCodeForMediaRetry = exports.decryptMediaRetryData = exports.decodeMediaRetryNode = exports.encryptMediaRetryRequest = exports.getWAUploadToServer = exports.downloadEncryptedContent = exports.downloadContentFromMessage = exports.getUrlFromDirectPath = exports.encryptedStream = exports.prepareStream = exports.getHttpStream = exports.getStream = exports.toBuffer = exports.toReadable = exports.getVideoDuration = exports.mediaMessageSHA256B64 = exports.generateProfilePicture = exports.encodeBase64EncodedStringForUpload = exports.extractImageThumb = exports.extractVideoThumb = exports.hkdfInfoKey = void 0;
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs, fileLength }) => {
        ...
        const isBuffer = Buffer.isBuffer(stream);
        const reqBody = isBuffer ? stream : stream;
        ...
        if (fileLength) {
            const sizedHosts = hosts.filter(({ maxContentLengthBytes }) => !maxContentLengthBytes || maxContentLengthBytes >= fileLength);
            if (sizedHosts.length) {
                candidateHosts = sizedHosts;
            }
        }
        if (!isBuffer && candidateHosts.length > 1) {
            logger.warn('streaming upload disables host retries');
            candidateHosts = candidateHosts.slice(0, 1);
        }
        for (const { hostname, maxContentLengthBytes } of candidateHosts) {
            logger.debug(`uploading to "${hostname}"`);
            const auth = encodeURIComponent(uploadInfo.auth);
            const url = `https://${hostname}${media}/${fileEncSha256B64}?auth=${auth}&token=${fileEncSha256B64}`;
            let result;
            try {
                if (maxContentLengthBytes && fileLength && fileLength > maxContentLengthBytes) {
                ...
                const body = await axios_1.default.post(url, reqBody, {
                    ...
                    headers: {
                        ...
                        'Origin': Defaults_1.DEFAULT_ORIGIN,
                        ...(fileLength ? { 'Content-Length': fileLength.toString() } : {})
                    },
