...
const makeChatsSocket = (config) => {
    ...
    let privacySettings;
    // ===== Initial sync state machine (ported from Baileys rc9) =====
    const SyncState = {
        Connecting: 0,
    ...
    */
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        // When we first receive pending notifications, chats.ts buffers events until either:
        // 1) we complete the first history/app-state sync, or
        // 2) we decide history sync is disabled / skipped, or
        // 3) we hit a timeout and force-flush.
    ...
    ev.on('connection.update', ({ connection, receivedPendingNotifications }) => {
        ...
        // Start the initial sync buffering once, right after the socket reports
        // that all pending notifications have been received.
