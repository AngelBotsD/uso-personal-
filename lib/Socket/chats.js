...
const makeChatsSocket = (config) => {
    ...
    let privacySettings;
    // ---- Sync state machine (ported from BailFork) ----
    const SyncState = {
    ...
    // ---- Dedicated mutexes (avoid receipts/notifications blocking message pipeline) ----
    ...
    /** backwards-compat: processingMutex refers to the message mutex */
    const processingMutex = messageMutex;
    ...
    */
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        const shouldProcessHistoryMsg = historyMsg
            ...
            : false;
        // State machine: decide on sync & flushing behaviour
        if (historyMsg && syncState === SyncState.AwaitingInitialSync) {
            ...
            if (shouldProcessHistoryMsg) {
                ...
                // doAppStateSync will flush once complete
            }
            else {
        ...
        const doAppStateSync = async () => {
            ...
            if (!authState.creds.myAppStateKeyId) {
                // we'll retry once we receive the key share
                logger.warn('App state key not present yet; deferring app state sync');
        ...
        if (((_c = (_b = msg.message) === null || _b === void 0 ? void 0 : _b.protocolMessage) === null || _c === void 0 ? void 0 : _c.appStateSyncKeyShare)
            && pendingAppStateSync) {
            // key share received; retry deferred app state sync
            await doAppStateSync();
    ...
    ws.on('CB:ib,,dirty', async (node) => {
        ...
        logger.info('History sync is enabled, awaiting notification with a 5s timeout.');
        if (awaitingSyncTimeout) {
            clearTimeout(awaitingSyncTimeout);
        }
