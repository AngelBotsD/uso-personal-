...
const makeChatsSocket = (config) => {
    ...
    let privacySettings;
    // Sync state machine (ported from BailFork) to avoid long startup buffering delays
    const SyncState = { Connecting: 0, AwaitingInitialSync: 1, Syncing: 2, Online: 3 };
    ...
    */
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        const shouldProcessHistoryMsg = historyMsg
            ...
            : false;
        // State machine: decide on initial sync vs. going Online immediately, and flush buffered events accordingly
        if (historyMsg && syncState === SyncState.AwaitingInitialSync) {
            ...
            if (shouldProcessHistoryMsg) {
                ...
                // let doAppStateSync handle the final flush once complete
            }
            else {
        ...
        const doAppStateSync = async () => {
            if (syncState === SyncState.Syncing) {
                ...
                // Sync complete — go online and flush everything
                syncState = SyncState.Online;
        ...
        // If the app state key arrives while we are waiting to sync, trigger the sync now.
