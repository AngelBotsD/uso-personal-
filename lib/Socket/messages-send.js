...
const makeMessagesSocket = (config) => {
    ...
    let _sessionFetchQueue = Promise.resolve();
    const withSessionFetchLock = (fn) => {
        const next = _sessionFetchQueue.then(fn, fn);
        _sessionFetchQueue = next.catch(() => { });
        return next;
    };
    let mediaConn;
    ...
    const assertSessions = async (jids, force) => withSessionFetchLock(async () => {
        var _a;
        ...
        // determine which sessions we are missing locally
        let jidsRequiringFetch = [];
        if (force) {
            jidsRequiringFetch = uniqueJids;
        }
        else {
            const addrs = uniqueJids.map(jid => signalRepository.jidToSignalProtocolAddress(jid));
        ...
        // Fetch in batches to avoid large IQ payloads that often time out in big groups
        const MAX_BATCH = 50;
        const QUERY_TIMEOUT_MS = 90_000;
        const batches = [];
        for (let i = 0; i < jidsRequiringFetch.length; i += MAX_BATCH) {
            batches.push(jidsRequiringFetch.slice(i, i + MAX_BATCH));
        }
        for (const batch of batches) {
            let attempt = 0;
            // retry a couple of times; if still failing, continue with remaining batches
            while (attempt < 3) {
                try {
                    logger.debug({ batchSize: batch.length, attempt: attempt + 1 }, 'fetching sessions');
                    const result = await query({
                        tag: 'iq',
                        attrs: {
                            xmlns: 'encrypt',
                    ...
                    const isTimeout = msg.includes('timed out') || msg.includes('request time-out') || (err === null || err === void 0 ? void 0 : err.output) && (err.output.statusCode === 408);
                    // If we're timing out, back off a bit and retry; otherwise surface the error
                    if (attempt < 3 && isTimeout) {
                        const backoff = 250 * Math.pow(2, attempt);
                        logger.warn({ err, attempt, backoff }, 'session fetch timed out; retrying');
                        await (0, Utils_1.delay)(backoff);
                        continue;
                    }
                    logger.warn({ err, batchSize: batch.length }, 'failed to fetch sessions');
                    break;
                }
            }
        }
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        ...
        const nodes = await Promise.all(patched.map(async (patchedMessageWithJid) => {
            ...
            let encResult;
            try {
                encResult = await signalRepository.encryptMessage({ jid, data: bytes });
            }
            catch (err) {
                // Common in busy groups / after reconnects: session missing or stale.
                // Force-refresh identity sessions once and retry encryption.
                const msg = String((err === null || err === void 0 ? void 0 : err.message) || err).toLowerCase();
                const looksLikeSessionIssue = msg.includes('no session') || msg.includes('no sessions') || (msg.includes('session') && msg.includes('not'));
                if (looksLikeSessionIssue) {
                    logger.warn({ err, jid }, 'encrypt failed due to session issue; force-refreshing session');
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (senderKeyJids.length) {
                    ...
                    // In large groups, a single sender-key fanout can involve hundreds of devices.
                    // Chunking avoids massive IQ payloads & encrypt work spikes that commonly lead to
                    // "Timed Out" during session fetch or message relay.
                    const MAX_SENDERKEY_BATCH = 50;
                    for (let i = 0; i < senderKeyJids.length; i += MAX_SENDERKEY_BATCH) {
                        const chunk = senderKeyJids.slice(i, i + MAX_SENDERKEY_BATCH);
                        await assertSessions(chunk, false);
                        const result = await createParticipantNodes(chunk, senderKeyMsg, extraAttrs);
