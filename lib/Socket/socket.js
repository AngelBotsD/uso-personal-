...
const makeSocket = (config) => {
    ...
    // Wait until the websocket is open before sending nodes (important for pairing-code flows
    // triggered immediately after socket construction, e.g. from a dashboard).
    const waitForSocketOpen = async () => {
        if (ws.isOpen) {
            return;
        }
        let onOpen;
        let onClose;
        await (0, Utils_1.promiseTimeout)(connectTimeoutMs, (resolve, reject) => {
            onOpen = resolve;
            onClose = mapWebSocketError(reject);
            ws.on('open', onOpen);
            ws.on('close', onClose);
...
const requestPairingCode = async (phoneNumber, customPairingCode) => {
        ...
        if (!normalizedPhone) {
            throw new Error('Invalid phone number for pairing code (expected digits incl. country code)');
        }
        let pairingCode;
        if (customPairingCode) {
            const cleaned = String(customPairingCode).trim().toUpperCase();
            if (cleaned.length !== 8) {
            ...
            pairingCode = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5));
        }
        authState.creds.pairingCode = pairingCode;
        authState.creds.me = {
            id: (0, WABinary_1.jidEncode)(normalizedPhone, 's.whatsapp.net'),
            name: '~'
        };
        ev.emit('creds.update', authState.creds);
        // Ensure socket is open before we try to send the MD iq, otherwise dashboards
        // that trigger pairing immediately can race the WS 'open' event.
