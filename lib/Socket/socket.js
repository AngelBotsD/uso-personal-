...
const makeSocket = (config) => {
    ...
    const isRateLimitError = (err) => {
        ...
        return msg.includes('rate limit') || msg.includes('rate-overlimit') || msg.includes('overlimit') || msg.includes('429') || code === 429;
    };
    const isTimeoutError = (err) => {
        const msg = String((err === null || err === void 0 ? void 0 : err.message) || err || '').toLowerCase();
        const code = (err === null || err === void 0 ? void 0 : err.status) || (err === null || err === void 0 ? void 0 : err.statusCode) || ((err === null || err === void 0 ? void 0 : err.output) && err.output.statusCode);
        return msg.includes('timed out') || code === 408 || code === Types_1.DisconnectReason.timedOut;
    };
    ...
    const query = async (node, timeoutMs) => {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            try {
                ...
                if (attempt >= maxAttempts - 1 || (!rateLimited && !timedOut)) {
                    throw err;
                }
                const disableBackoff = config.backoffDelayMs === 0;
                const baseDelay = config.backoffDelayMs ?? 5000;
                const delayMs = disableBackoff
                    ? 0
                    : rateLimited
                        ? Math.min(30_000, baseDelay * Math.pow(2, attempt))
                        : Math.min(2_000, 500 * (attempt + 1));
                if (delayMs > 0) {
                    ws.setBackoff?.(delayMs);
