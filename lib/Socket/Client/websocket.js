...
class WebSocketClient extends types_1.AbstractSocketClient {
    ...
    setBackoff(waitMs) {
        const delay = Math.min(this._maxBackoffMs, Math.max(0, waitMs || 0));
        if (!delay) {
            return;
        }
        const now = Date.now();
        this._backoffMs = Math.min(this._maxBackoffMs, Math.max(this._backoffMs, delay));
        this._backoffUntil = Math.max(this._backoffUntil, now + this._backoffMs);
    }
    ...
    _dispatch() {
        ...
        const tick = () => {
            ...
            const backoffWait = Math.max(0, this._backoffUntil - now);
            const wait = Math.max(backoffWait, this._minSendIntervalMs - delta, 0);
            setTimeout(() => {
