...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, processingMutex, messageMutex, receiptMutex, notificationMutex, signalRepository, query, upsertMessage, resyncAppState, groupMetadata, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, createParticipantNodes, getUSyncDevices, sendPeerDataOperationMessage, } = sock;
    /** this mutex ensures that each retryRequest will wait for the previous one to finish */
    const retryMutex = (0, make_mutex_1.makeMutex)();
    const messageProcessingMutex = enableParallelMessageProcessing
        ? (0, make_mutex_1.makeKeyedMutex)()
        : { mutex: (_key, task) => ((messageMutex || processingMutex).mutex(task)) };
    ...
    const handleReceipt = async (node) => {
        ...
        try {
            await Promise.all([
                (receiptMutex || processingMutex).mutex(async () => {
    ...
    const handleNotification = async (node) => {
        ...
        try {
            await Promise.all([
                (notificationMutex || processingMutex).mutex(async () => {
                    var _a;
